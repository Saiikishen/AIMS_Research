...
OUTPUT_DIR    = 'data'
IMAGE_DIR     = os.path.join('data', 'images1')
...
# ── RUNTIME PROMPT ────────────────────────────────────────────────────────────
def prompt_info():
    dlg = gui.Dlg(title='Picture Naming')
...
def init_log(subj, ses, day, run):
    ...
    fn = os.path.join(OUTPUT_DIR,
         f'task5_word_generation_{subj}_ses{ses}_day{day}_run{run}_{ts}.csv')
